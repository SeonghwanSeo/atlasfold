...
def triangle_attn_forward(
    ...
    *,
    ...
    tuned_shape = C_in == 128 and H == 4 and D == 32
    bias_config = dict(BIAS_CONFIG if tuned_shape else _DEFAULT_BIAS_CONFIG)
    ...
    attention_config = dict(
        ATTENTION_CONFIG if tuned_shape else _DEFAULT_ATTENTION_CONFIG
    )
...
# Common fixed C=128/H=4 settings from the L40S sweep, independent of length.
BIAS_CONFIG = dict(BLOCK_QK=128, num_warps=8, num_stages=2)
ATTENTION_CONFIG = dict(BLOCK_M=256, BLOCK_K=128, num_warps=8, num_stages=2)
_DEFAULT_BIAS_CONFIG = dict(BLOCK_QK=128, num_warps=4, num_stages=2)
_DEFAULT_ATTENTION_CONFIG = dict(BLOCK_M=64, BLOCK_K=32, num_warps=4, num_stages=2)

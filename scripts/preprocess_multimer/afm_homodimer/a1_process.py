...
from atlasfold.common import protein, protein_complex, residue_constants
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default=None,
        help="Path to a text file containing keys of structures",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
...
def read_protein_structure(
    ...
    for raw_chain in structure[0]:
        chain_name = f"{name}_{raw_chain.name}"
        # Get sequence
        residue_dict = {}
        for res in raw_chain:
            ...
            if res_idx is None:
                logger.warning(
                    f"Residue {res.name} in {chain_name} missing res idx; skipping."
                )
                continue
...
def main():
    ...
    data_dir: pathlib.Path = args.data_dir / "npz/"
    data_dir.mkdir(parents=True, exist_ok=True)

    if args.key_path is not None:
        with open(args.key_path) as f:
            filtered_keys = set(line.strip() for line in f)
        logger.info(f"Filtering to {len(filtered_keys)} structures based on metadata.")
    else:
        filtered_keys = None
    ...
    for subdir in (pbar := tqdm(sorted(pdb_dir.iterdir()))):
        ...
        if filtered_keys is not None:
            pdb_paths = [
                path for path in pdb_paths if path.name.split(".")[0] in filtered_keys
            ]
            logger.info(
                f"After filtering, {len(pdb_paths)} structures remain in {subdir}."
            )
        ...
        with multiprocessing.Pool(args.num_workers) as pool:
            results = list(
                tqdm(
                    pool.imap_unordered(parse_pdb_partial, pdb_paths, chunksize=5),
                    total=len(pdb_paths),

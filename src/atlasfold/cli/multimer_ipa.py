...
def run(args: argparse.Namespace) -> None:
    ...
    def load_model(
        ...
        if model_path is None:
            raise ValueError("A local multimer IPA model path is required.")
    ...
    def write_outputs(
        ...
        with open(out_dir / f"{output.name}_summary.csv", "w") as f:
            f.write("seed,ranking_score,ptm,iptm,avg_plddt,num_recycles\n")
    ...
    logger.info(
        "Starting multimer IPA inference: seeds=%s, num_recycles=%d, "

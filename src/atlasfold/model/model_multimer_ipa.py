...
class AtlasFoldMultimerIPAConfig:
    name: str = "atlasfold-multimer-ipa"
...
class AtlasFoldMultimer_IPA(torch.nn.Module):
    ...
    def inference(
        ...
        batch: dict[str, torch.Tensor]
            ...
            - "aatype"          : [B, L, 21] float
                One-hot encoded amino acid types for concatenated chains.
            - "aatype_int"      : [B, L] int
                Amino acid type indices.
            - "res_idx"         : [B, L] int
                Residue indices within each chain.
            - "asym_id"         : [B, L] int
                Chain IDs.
            - "entity_id"       : [B, L] int
                Entity IDs.
            - "sym_id"          : [B, L] int
                Copy IDs within each entity.
            - "seq_tok_idx"     : [B, L] int
    ...
    def from_pretrained(
        ...
        *,
        ...
        """Create an AtlasFold multimer IPA model from pretrained weights."""
        if config is None:
            config = AtlasFoldMultimerIPAConfig()

...
def _validate_projection_inputs(
    ...
    if not x.is_cuda:
        raise RuntimeError("Triangle attention epilogue requires CUDA tensors")
...
def _gated_output_projection(
    ...
    *,
    ...
    config = dict(GATE_CONFIG if channel == 128 else _DEFAULT_GATE_CONFIG)
...
__all__ = ["gated_output_projection_blc"]

# Common fixed C=128 setting from the L40S sweep.
GATE_CONFIG = dict(BLOCK_M=64, BLOCK_N=128, BLOCK_K=32, num_warps=8, num_stages=3)
_DEFAULT_GATE_CONFIG = dict(BLOCK_M=64, BLOCK_N=64, BLOCK_K=32, num_warps=4, num_stages=2)

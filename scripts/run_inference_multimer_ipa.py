"""Run AtlasFold multimer IPA inference."""
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mlm-prob", type=float, default=0.20)
    parser.add_argument(
...
def load_inputs(path: Path) -> list[MultimerIPAFoldingInput]:
    ...
    for header, sequence in read_fasta(path):
        chains = [part for part in sequence.split(":") if part.strip()]
        if not chains:
            raise ValueError(f"FASTA target {header!r} has no non-empty chains.")
        inputs.append(MultimerIPAFoldingInput(header.split()[0].strip(), chains))
    if not inputs:
...
def load_model(args: argparse.Namespace) -> AtlasFoldMultimer_IPA:
...
def _log_recycle(name: str, seed: int, recycle: int, record: dict) -> None:
    ...
    logger.info(
        "%s seed=%d recycle=%d tol=%s pLDDT=%.3f pTM=%.3f "
        "ipTM=%.3f rank=%.3f avg_PAE=%.3f",
        name,
        seed,
        recycle,
        tol_text,
...
def write_outputs(
    out_dir: Path,
    output: MultimerIPAFoldingOutput,
    *,
    format: str,
    ...
    with (out_dir / f"{output.name}_summary.csv").open("w") as handle:
        handle.write("seed,ranking_score,avg_plddt,ptm,iptm,num_recycles,final_tol\n")
        for record in records:
            ...
            handle.write(
                ...
                f"{record['iptm']:.3f},{record['num_recycles']},{final_tol}\n"
            )
    done_path.touch()
...
def run(args: argparse.Namespace) -> None:
    ...
    for outputs in runner.fold_iter_batch(
        ...
        for output in outputs:
            ...
            completed += 1
            logger.info(
                "Completed %s: pLDDT=%.3f pTM=%.3f ipTM=%.3f recycles=%d (%d/%d)",
                output.name,
                best["avg_plddt"],
                best["ptm"],
                best["iptm"],

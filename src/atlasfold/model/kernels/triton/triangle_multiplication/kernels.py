...
def fused_layer_norm_sigmoid_gated_transpose(
    ...
    for k in tl.range(0, D_OUT, BLOCK_K):
        ...
        P = tl.dot(X1_norm, Wp)
        G = tl.dot(X2_norm, Wg)
        if HAS_MASK and TWO_INPUTS:
            G = tl.where(m_val[:, None] != 0, G, 0.0)
        Out = tl.sigmoid(G) * P
        if HAS_MASK and not TWO_INPUTS:
            Out = Out * m_val[:, None]
...
def _launch(
    ...
    *,
    ...
    tuned_shape = D == 128 and D_OUT == (128 if two_inputs else 256)
    if tuned_shape:
        config = dict(OUTPUT_CONFIG if two_inputs else INPUT_CONFIG)
    else:
        config = dict(_DEFAULT_OUTPUT_CONFIG if two_inputs else _DEFAULT_INPUT_CONFIG)
...
def output_phase(
    ...
    _launch(
        ...
        norm_in_weight,
        norm_in_bias,
        mask.reshape(M).to(torch.int8).contiguous() if mask is not None else None,
        out,
        out.stride(0),
        out.stride(1),
        M,
...
# Common fixed C=128 settings from the L40S sweep, shared by both directions.
INPUT_CONFIG = dict(BLOCK_M=128, BLOCK_K=32, num_warps=8, num_stages=1)
OUTPUT_CONFIG = dict(BLOCK_M=64, BLOCK_K=64, num_warps=4, num_stages=1)
_DEFAULT_INPUT_CONFIG = dict(BLOCK_M=64, BLOCK_K=32, num_warps=4, num_stages=2)
_DEFAULT_OUTPUT_CONFIG = dict(BLOCK_M=32, BLOCK_K=32, num_warps=4, num_stages=1)
